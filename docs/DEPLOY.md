...
Every `/reconcile` and `/invoke` run creates a top-level span. Nested spans include:

- LangGraph nodes (via LangChain callback handler)
- Claude model calls
- MCP tool calls (`mcp.parse_invoice`, `mcp.reconcile`, etc.)

View traces at [braintrust.dev](https://www.braintrust.dev) in the `verity` project.

View traces at [braintrust.dev](https://www.braintrust.dev) in the `verity` project. When tracing is enabled, `/reconcile` returns a `braintrust_trace_url` deep link — the dashboard surfaces it after each run.
...
No Anthropic or Braintrust keys on Vercel — the frontend never calls Claude directly; it proxies to the agent service.
...
Open the deployed dashboard, run reconciliation, and click **View trace in Braintrust** (requires `BRAINTRUST_API_KEY` on the agent).

### CORS / networking

The Next.js API routes (`/api/reconcile`, `/api/chat`) call the agent server-side, so browser CORS is not an issue. Ensure the Railway agent URL is reachable from Vercel's build/runtime network (public Railway URL works).
